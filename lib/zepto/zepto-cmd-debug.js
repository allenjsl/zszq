    ...
    $.fn.show = function (timer) {
        ...
        if (timer) {
            var nId = setTimeout(function () {
                var ss = timer / 1000;
                oThis.css('transition', ss + 's');
                oThis.css('height', ''); clearTimeout(nId);
            }, 10);
            return;
        }
        else {
            oThis.css('transition', '');
            oThis.css('height', '');
        }
    }
    /**
   *@说明：隐藏当前元素
   *@param {Number} 隐藏的动画时间
   */
    $.fn.hide = function (timer) {
        if (timer) {
            var ss = timer / 1000;
            this.css('transition', ss + 's');
            this.css('height', '0px'); return;
        }
        else {
            this.css('height', '0px');
    ...
    */
    ...
    $.extend($, new TZT($));
    window.$ = undefined; window.Zepto = undefined; module.exports = Zepto;
});
